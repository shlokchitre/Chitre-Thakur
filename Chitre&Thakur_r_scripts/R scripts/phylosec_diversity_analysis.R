...
z.int <- transform_sample_counts(z.int, function(ASV) ASV/sum(ASV) *100 )


#Barplot 
plot_bar(z.int, x= "SampleID", fill="Genus") + facet_wrap(~Interaction, scales= "free_x", nrow=1)+
...
theme_classic()
pcoa_zint = ordinate(z.int, "PCoA", "wunifrac")
pcoa_zintbray = ordinate(z.int, "PCoA", "bray")

pcoa_int <- plot_ordination(z.int, pcoa_zint , type="samples", color = "Interaction") 
themed_pcoa <- pcoa_int + geom_point(size = 2) + geom_polygon(aes(fill=Interaction ), alpha=0.2)  + ggtitle("PCoA plot of ASV profiles")
themed_pcoa + theme_classic()

nmds_zint = ordinate(z.int, "NMDS", "wunifrac") 
nmds_int <- plot_ordination(z.int, nmds_zint , type="samples", color = "Interaction") 
themed_nmds <- nmds_int + geom_point(size = 2) + geom_polygon(aes(fill=Interaction ), alpha=0.2) + ggtitle("NMDS plot of ASV profiles") 
themed_nmds + theme_classic()
